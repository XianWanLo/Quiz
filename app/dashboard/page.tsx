...
const StatsPage: React.FC = () => {
  ...
  const generateColors = (count: number) => {
    ...
    const q8totalCount = groupedQuestions['8'].reduce((sum: number, opt: any) => sum + opt.count, 0);

    // Formatter function to calculate percentage
    const percentageFormatter = (value: number) => {
      return q8totalCount > 0 ? `${((value / q8totalCount) * 100).toFixed(1)}%` : '0%';
    };

    // Prepare data for Output Repetition (Pie Chart)
    const q8Data = {
      labels: groupedQuestions['8'].map((opt: any) => opt.selectedAnswer),
      datasets: [
        {
          data: groupedQuestions['8'].map((opt: any) => opt.count),
          backgroundColor: generateColors(groupedQuestions['8'].length),  // Generate colors dynamically
          borderColor: generateColors(groupedQuestions['8'].length).map(color => color.replace('0.6', '1')),  // Opaque borders
          borderWidth: 1,
        },
      ],
    };

    const q8pieChartOptions = {
      plugins: {
        legend: {
  ...
  const pageViewsData = {
    labels: stats.pageViews.map((page: any) => page.pageName),  // Pages (e.g., home, about, etc.)
    datasets: [
      {
        label: "Page Views",
  ...
  const responseTimeData = {
    labels: stats.avgResponseTimeByPage.map((page: any) => page.pageName),
    datasets: [
      {
        label: "Average Response Time (ms)",
        data: stats.avgResponseTimeByPage.map((page: any) => page._avg.responseTime.toFixed(3)),
  ...
  Logout
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="bg-white shadow-lg rounded-lg p-6">
          ...
          <h3 className="text-xl font-semibold mb-2">Question: 8</h3>
            <Pie data={q8Data} options={q8pieChartOptions} />
        </div>
 ...
 <div className="bg-white shadow-lg rounded-lg p-6">
        ...
        {/* <h3 className="text-xl font-semibold mb-2">Question 8</h3>
          <div className="w-96 h-96">
            <Pie data={q8Data} options={pieChartOptions} />
          </div> */}
